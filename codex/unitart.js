/* Polyforge — procedural character art, realistic pass.
   ...
   rather than flat fills. Mounts to the isometric stage in src/render/renderer.ts. */
(function () {
  const TRIBES = [
    { id: "ashfen", name: "Ashfen", color: "#e05c4b", colorDark: "#8f3527" },
    { id: "korvani", name: "Korvani", color: "#4b8fe0", colorDark: "#2b5c9e" },
    { id: "meridia", name: "Meridia", color: "#e0b84b", colorDark: "#9e7d27" },
    { id: "thornwood", name: "Thornwood", color: "#5cb85c", colorDark: "#357a35" },
  ];

  const UNITS = {
    warrior:   { name: "Warrior",   atk: 2, def: 2, mov: 1, hp: 10, range: 1, cost: 2,  tech: null },
    rider:     { name: "Rider",     atk: 2, def: 1, mov: 2, hp: 10, range: 1, cost: 3,  tech: "riding" },
    archer:    { name: "Archer",    atk: 2, def: 1, mov: 1, hp: 10, range: 2, cost: 3,  tech: "archery" },
    defender:  { name: "Defender",  atk: 1, def: 3, mov: 1, hp: 15, range: 1, cost: 3,  tech: "shields" },
    swordsman: { name: "Swordsman", atk: 3, def: 3, mov: 1, hp: 15, range: 1, cost: 5,  tech: "smithery" },
    catapult:  { name: "Catapult",  atk: 4, def: 0, mov: 1, hp: 10, range: 3, cost: 8,  tech: "mathematics" },
    knight:    { name: "Knight",    atk: 4, def: 1, mov: 3, hp: 15, range: 1, cost: 8,  tech: "chivalry" },
    giant:     { name: "Giant",     atk: 5, def: 4, mov: 1, hp: 40, range: 1, cost: 10, tech: null },
  };

  /* ───────────── color ───────────── */
  function rgb(h) {
    if (h.charAt(0) !== "#") { const m = h.match(/-?\d+/g) || [0, 0, 0]; return [+m[0], +m[1], +m[2]]; }
    h = h.replace("#", "");
  ...
  const lt = (c, t) => mix(c, "#fff6e8", t);
  const dk = (c, t) => mix(c, "#120e14", t);
  const alpha = (c, a) => { const A = rgb(c); return "rgba(" + A.join(",") + "," + a + ")"; };
  ...
  function G(ctx, x0, y0, x1, y1, stops) {
  ...
  function limb(ctx, a, b, r1, r2, col, opts) {
    opts = opts || {};
  ...
  function plate(ctx, pts, col, opts) {
    opts = opts || {};
  ...
  function orb(ctx, x, y, rx, ry, col, opts) {
    opts = opts || {};
    ctx.beginPath();
  ...
  function ao(ctx, x, y, r, strength) {
    ctx.fillStyle = RG(ctx, x, y, r, [[0, "rgba(10,6,12," + (strength || 0.35) + ")"], [1, "rgba(10,6,12,0)"]]);
    ctx.beginPath();
  ...
  function spec(ctx, x, y, rx, ry, rot, a) {
    ctx.save();
  ...
  function stroke(ctx, pts, col, w, cap) {
    ctx.beginPath();
    ctx.moveTo(pts[0][0], pts[0][1]);
    for (let i = 1; i < pts.length; i++) ctx.lineTo(pts[i][0], pts[i][1]);
    ctx.strokeStyle = col; ctx.lineWidth = w; ctx.lineCap = cap || "round"; ctx.lineJoin = "round";
    ctx.stroke();
    ctx.lineCap = "butt";
  }

  /** Cloth panel with hanging folds. */
  function cloth(ctx, pts, col, folds) {
  ...
  /** Wood with grain. */
  function wood(ctx, pts, opts) {
    plate(ctx, pts, MAT.wood, Object.assign({ hi: MAT.woodHi, lo: MAT.woodLo }, opts || {}));
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(pts[0][0], pts[0][1]);
    for (let i = 1; i < pts.length; i++) ctx.lineTo(pts[i][0], pts[i][1]);
  ...
  const KITS = {
    ashfen: {
      ...
      shield(ctx, k) { // teardrop, flame-notched rim
    ...
    korvani: {
      ...
      helm(ctx, k) { // finned diving helm with a glass port
      ...
      shield(ctx, k) { // round, scalloped, wave boss
    ...
    meridia: {
      ...
      helm(ctx, k) { // gilt parade helm, transverse crest, sun disc
      ...
      shield(ctx, k) { // tower shield with sunburst
    ...
    thornwood: {
      ...
      helm(ctx, k) { // antlered bark hood over a carved leaf mask
        ctx.beginPath();
        ctx.moveTo(-10, 4);
      ...
      shield(ctx, k) { // lashed bark planks
  ...
  function kitFor(tribeId) {
    const t = TRIBES.find((t) => t.id === tribeId) || TRIBES[0];
    const k = Object.assign({}, KITS[t.id]);
    k.color = t.color; k.dark = t.colorDark; k.name = t.name; k.id = t.id;
    k.armor = t.color; k.armorDark = t.colorDark;
    return k;
  }

  /* ───────────── the human figure ─────────────
     100 units tall; 3/4 view facing right; key light upper-left. */
  function head(ctx, k, cx, cy, s) {
    ctx.save();
    ctx.translate(cx, cy);
    ctx.scale(s, s);
  ...
  /**
   * cfg: { bulk, crouch, lean, cape, plated, armSpread }
   * Returns joint positions so weapons/shields can be hung on the figure.
   */
  function body(ctx, k, cfg) {
    cfg = cfg || {};
  ...
  function arm(ctx, k, from, to, elbow, bulk, opts) {
    opts = opts || {};
  ...
  /* ───────────── weapons ───────────── */
  function haftStyle(ctx, k, a, b, w) {
    if (k.weapon === "thorn") {
  ...
  function bladeGeom(ctx, k, len, w) {
    if (k.weapon === "notched") {
  ...
  function sword(ctx, k, x, y, rot, len, hilt) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rot);
  ...
  function spear(ctx, k, x, y, rot, len) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rot);
  ...
  function horse(ctx, k, opts) {
    ...
    // hooves
    [[-14, -1.5], [16, -1.5], [-19, -1.5], [20, -1.5]].forEach((p) => plate(ctx, [[p[0] - 3.4, p[1] - 2.5], [p[0] + 3.4, p[1] - 2.5], [p[0] + 3.8, p[1] + 1.5], [p[0] - 3.8, p[1] + 1.5]], "#3a332c", { hi: "#6c6157" }));
    // neck + head
  ...
  const BUILD = {
    ...
    catapult(ctx, k) {
      ...
      // iron bands
      [[-26, -22], [30, -22]].forEach((p) => plate(ctx, [[p[0] - 5, p[1] - 3], [p[0] + 5, p[1] - 3], [p[0] + 5, p[1] + 2], [p[0] - 5, p[1] + 2]], MAT.iron, { hi: MAT.ironHi, spec: true }));
      // torsion bundle
  ...
  function drawCharacter(ctx, opts) {
    if (opts.acted) { drawActed(ctx, opts); return; }
    paintCharacter(ctx, opts, opts.x || 0, opts.y || 0);
  }

  /* Acted units dim their own pixels only: paint into an offscreen buffer, tint
     it there with source-atop, then blit. Tinting on the shared map context
     would darken the terrain under the figure. */
  const BOX = { w: 220, h: 260, ox: 110, oy: 232 };
  function drawActed(ctx, opts) {
    const scale = (opts.scale == null ? 1 : opts.scale) * FIT;
    const ss = 2;
    const off = document.createElement("canvas");
    off.width = Math.max(1, Math.ceil(BOX.w * scale * ss));
    off.height = Math.max(1, Math.ceil(BOX.h * scale * ss));
    const o = off.getContext("2d");
    o.scale(scale * ss, scale * ss);
    paint(o, opts, BOX.ox, BOX.oy, 1);
    o.globalCompositeOperation = "source-atop";
    o.fillStyle = "rgba(18,24,34,0.45)";
    o.fillRect(0, 0, BOX.w, BOX.h);
    o.globalCompositeOperation = "source-over";
    ctx.drawImage(off, (opts.x || 0) - BOX.ox * scale, (opts.y || 0) - BOX.oy * scale, BOX.w * scale, BOX.h * scale);
  }

  function paintCharacter(ctx, opts, px, py) {
    paint(ctx, opts, px, py, (opts.scale == null ? 1 : opts.scale) * FIT);
  }

  function paint(ctx, opts, px, py, scale) {
    ...
    ctx.lineJoin = "round";
    ctx.save();
    if (k.lean) { ctx.translate(0, 0); ctx.transform(1, 0, -k.lean, 1, 0, 0); }
